...
class Equipos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre, numero, Estado, marca, modelo, serial, voltaje, ubicacion', 'required'),
			array('linea_servicio_id, numero', 'numerical', 'integerOnly'=>true),
			array('nombre', 'length', 'max'=>40),
			array('Estado, voltaje', 'length', 'max'=>10),
			array('marca, modelo', 'length', 'max'=>25),
			array('serial, ubicacion', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre, linea_servicio_id, numero, Estado, marca, modelo, serial, voltaje, ubicacion', 'safe', 'on'=>'search'),
		);
	}
