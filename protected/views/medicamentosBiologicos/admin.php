...
$('.search-form form').submit(function(){
	$('#medicamentos-biologicos-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<h1>Buscar Medicamentos Biológicos - <a href="#exportar" class="btn btn-warning" role="button" data-toggle="modal"><i class="icon-share-alt icon-white"></i> Exportar</a></h1>
...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'medicamentos-biologicos-grid',
...
<div id="exportar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
 	...
 	<form id="frmExportar" name="frmExportar" action="index.php?r=medicamentosBiologicos/exportar&tipo=<?php $elTipo;?>" method = "post">
  		<div class="span12">
			<label>Filtro:</label>
...
<script>
    ...
    {
        $('body').on('dblclick', '#medicamentos-biologicos-grid tbody tr', function(event)
        {
            var
                rowNum = $(this).index(),
                keys = $('#medicamentos-biologicos-grid > div.keys > span'),
                rowId = keys.eq(rowNum).text();

            location.href = '<?php echo Yii::app()->createUrl('medicamentosBiologicos/view'); ?>&id=' + rowId;
        });
    });
</script>