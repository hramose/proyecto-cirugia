...
<h1>Buscar Laboratorios - <a href="#exportar" class="btn btn-warning" role="button" data-toggle="modal"><i class="icon-share-alt icon-white"></i> Exportar</a></h1>
...
<div id="exportar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
 	...
 	<form id="frmExportar" name="frmExportar" action="index.php?r=laboratorio/exportar&tipo=<?php $elTipo;?>" method = "post">
  		<div class="span5">
			<label>Filtro:</label>
...
<script>
    ...
    {
        $('body').on('dblclick', '#laboratorio-grid tbody tr', function(event)
        {
            var
                rowNum = $(this).index(),
                keys = $('#laboratorio-grid > div.keys > span'),
                rowId = keys.eq(rowNum).text();

            location.href = '<?php echo Yii::app()->createUrl('laboratorio/view'); ?>&id=' + rowId;
        });
    });
</script>