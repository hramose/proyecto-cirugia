...
$('.search-form form').submit(function(){
	$('#formulacion-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<h1>Productos de Formulación - <a href="#exportar" class="btn btn-warning" role="button" data-toggle="modal"><i class="icon-share-alt icon-white"></i> Exportar</a></h1>
...
<div id="exportar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
 	...
 	<form id="frmExportar" name="frmExportar" action="index.php?r=formulacion/exportar&tipo=<?php $elTipo;?>" method = "post">
  		<div class="span12">
			<label>Filtro:</label>
...
<script>
    ...
    {
        $('body').on('dblclick', '#formulacion-grid tbody tr', function(event)
        {
            var
                rowNum = $(this).index(),
                keys = $('#formulacion-grid > div.keys > span'),
                rowId = keys.eq(rowNum).text();

            location.href = '<?php echo Yii::app()->createUrl('formulacion/view'); ?>&id=' + rowId;
        });
    });
</script>
