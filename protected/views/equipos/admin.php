...
<div id="exportar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
 	...
 	<form id="frmExportar" name="frmExportar" action="index.php?r=equipos/exportar&tipo=<?php $elTipo;?>" method = "post">
  		<div class="span5">
			<label>Filtro:</label>
...
<script>
    ...
    {
        $('body').on('dblclick', '#equipos-grid tbody tr', function(event)
        {
            var
                rowNum = $(this).index(),
                keys = $('#equipos-grid > div.keys > span'),
                rowId = keys.eq(rowNum).text();

            location.href = '<?php echo Yii::app()->createUrl('equipos/view'); ?>&id=' + rowId;
        });
    });
</script>