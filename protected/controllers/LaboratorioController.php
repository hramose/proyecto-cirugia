...
class LaboratorioController extends Controller
{
	...
	{
		$dataProvider=new CActiveDataProvider('Laboratorio');
	...
	public function actionExportar()
	{
		if ($_POST['filtro'] == 1) 
		{
			...
			$rows = Laboratorio::model()->findAllByAttributes($attribs, $criteria);
		}
		else
		{
			$rows = Laboratorio::model()->findAll();
		}
	...
	{
		$model=new Laboratorio('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Laboratorio']))
			$model->attributes=$_GET['Laboratorio'];
	 ...
	 * @param Laboratorio $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='laboratorio-form')
		{
